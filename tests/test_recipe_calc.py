...
合计由 demand_leaves/demand_byproducts 等推导函数按制造次数重算。
...
def solve_leaves(graph, target, qty, **kw):
    """单目标求解便利:返回 (平面解, 需求叶子清单)。"""
    res = compute(graph, target, Fraction(qty), **kw)
    return res, demand_leaves(graph, {target: Fraction(qty)}, res.crafts)
...
def test_vaporizer_produce_env(graph):
    ...
    res = compute(graph, "item_copper_enr2_cmpt", Fraction(6))
    assert not any(rid.startswith("vaporizer_") for rid in res.crafts)


# ---------------------------------------------------------------- 基础求解
def test_basic_chain(graph):
    res, leaves = solve_leaves(graph, "item_iron_cmpt", 10)
...
def test_fractional_qty(graph):
    res, leaves = solve_leaves(graph, "item_iron_nugget", Fraction(1, 2))
...
def test_seed_loop_native(graph):
    ...
    清水(水泵采集)为唯一外部投入。"""
    res, leaves = solve_leaves(graph, "item_plant_grass_1", 10)
    assert res.strict_ok
    assert res.crafts == {
    ...
    assert demand_byproducts(graph, {"item_plant_grass_1": Fraction(10)},
                             res.crafts) == ({}, {})
...
def test_available_stops_production(graph):
    ...
    assert demand_leaves(graph, targets, res.crafts) == {
...
def test_gatherable_water_external(graph):
    ...
    res, leaves = solve_leaves(graph, "item_copper_nugget", 1)
    ...
    assert demand_byproducts(graph, {"item_copper_nugget": Fraction(1)}, res.crafts) == (
...
def test_spaceship_free_recipe(graph):
    """飞船配方无原料:制造步骤成立但零投入。"""
    res, leaves = solve_leaves(graph, "item_expcard_stage1_low", 5)
    assert leaves == {}
...
def test_preferred_discount(graph):
    ...
    assert required_envs(graph.recipes_by_id, default.crafts) == []
    res = Z3Solver(graph.recipes).solve(
    ...
    assert required_envs(graph.recipes_by_id, res.crafts) == [
        ("稳定环境", ["xiranite_oven_xiranite_powder_2"])]
    leaves = demand_leaves(graph, {"item_xiranite_powder": Fraction(1)}, res.crafts)
...
def test_rate_filter_core_60_per_min(graph):
    ...
    res, leaves = solve_leaves(graph, "item_filter_core", 60, per_min=True)
    assert leaves == {
    ...
    assert demand_byproducts(graph, {"item_filter_core": Fraction(60)},
                             res.crafts)[0] == {"item_liquid_sewage": Fraction(60)}
    machines = {r.id: m for r, _, m in machine_counts(graph.recipes_by_id, res.crafts)}
...
def test_rate_copper_enr2_6_per_min_provided(graph):
    ...
    mats = materials(demand_leaves(graph, targets, res.crafts))
    ...
    envs = dict(required_envs(graph.recipes_by_id, res.crafts))
...
def test_byproducts_flag(graph):
    ...
    assert ok.strict_ok
    assert demand_byproducts(graph, targets, ok.crafts) == ({}, {})
...
def test_conservation_sample(graph):
    ...
    for t in sample:
        res = Z3Solver(graph.recipes).solve(SolveRequest({t: Fraction(7)}))
        assert res.strict_ok, t
        flows = net_flows(graph.recipes_by_id, res.crafts)
        for (iid, _kind), total in demand_leaves(graph, {t: Fraction(7)}, res.crafts).items():
...
def test_solver_abstraction(graph):
    ...
    via_class = Z3Solver(graph.recipes).solve(
    ...
    targets = {"item_filter_core": Fraction(60)}
    assert (demand_leaves(graph, targets, via_compute.crafts)
            == demand_leaves(graph, targets, via_class.crafts))
...
def test_mermaid_flow(graph):
    ...
    res = compute(graph, *next(iter(targets.items())))
    text = render_mermaid(graph, targets, res.crafts)
...
def test_mermaid_upkeep_edge(graph):
    ...
    targets = {"item_filter_core": Fraction(60)}
    res = compute(graph, "item_filter_core", Fraction(60), per_min=True)
    text = render_mermaid(graph, targets, res.crafts)
...
def test_render_rate_and_batch(graph):
    targets = {"item_filter_core": Fraction(60)}
    res = compute(graph, "item_filter_core", Fraction(60), per_min=True)
    text = render_result(graph, "分离芯", targets, res, per_min=True)
    assert "×60/min" in text and "设备需求" in text
    batch_targets = {"item_filter_core": Fraction(4)}
    batch = render_result(graph, "分离芯", batch_targets,
...
def test_render_sections(graph):
    ...
    text = render_result(graph, "灼铜零件", targets, res, per_min=True)
...
def test_result_json(graph):
    ...
    payload = json.loads(json.dumps(result_json(graph, targets, res, per_min=True)))
