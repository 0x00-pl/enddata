...
定义侧不入产物,数字 id 组只保留各表的引用侧。Json 下关卡侧资源目录(关卡/
地图/导航/脚本/刷怪/任务运行时/NPC/口型/动画/交互等 19 个目录)同样已入
黑名单 —— 即使 `--all-json` 也不会进入。
...
- 已知边界:单路径组(rid=0)没有规则;数组不记下标;JSON 空键被分词剔除;
  混合大小写 map 键不作键出现(见定位符规范);关卡侧资源目录在黑名单中,
  `--dir` 也无法带入;变量绑定不校验同值,覆盖判定在极端情形略宽松。
