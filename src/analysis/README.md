...
- **recipe_calc.py — 配方用料倒推**(`enddata analysis recipe [ITEM] [QTY] [--have 物品,…] [--json]`):
  ...
  **环境维持**:气体散布机(FactoryVaporizerTable)持续通入对应气体 6/min
  ...
  数量支持速率口径(如 `60/min`);求解抽象为类体系:`RecipeSolver` 抽象基类 +
  `RecursiveSolver` 默认实现(严格展开/外部投料迭代增长收缩/宽松兜底),新求解器
  实现 `solve()` 后在 `SOLVERS` 注册即可经 `--solver` 使用;
  已内置 `z3` 求解器(`solvers/z3.py`,SMT/LP 整图一次求解:环与共享中间品天然
  可解、采集类资源免费用量、目标最少制造次数;环境维持计入采集气体,设备维持
  与 preferred 暂未生效;byproducts=False 为硬约束:依赖副产物的路线不可行);
  无参数生成 `reports/recipe-analysis.md`(数据概览 + 潜在循环依赖组 + 示例倒推)。
  正确性:对全部 285 个可制造物品做了流量守恒审计(产出=单产×次数、
  子需求=单耗×次数、宏配方环内守恒、目标净产=需求量),0 违规;
  单元测试见 `tests/test_recipe_calc.py`(poetry run pytest,38 用例)。
