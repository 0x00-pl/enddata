...
_DESC_TERMS = (
    ...
    "物理异常", "法术异常", "物理脆弱", "法术脆弱", "涡流", "法术爆发", "失衡",
    "灼热脆弱", "寒冷脆弱", "自然脆弱", "电磁脆弱",
    "重击", "连击", "猛击", "倒地", "击飞", "腐蚀",
    # 全队增幅(施加给干员一方的增益)
    "灼热增幅", "寒冷增幅", "自然增幅", "电磁增幅",
    # 干员特殊资源与召唤物(骏卫铁誓/莱万汀熔火/庄方宜青霆剑/赛希支援晶体/
    # 大潘备料/狼卫灼热獠牙等)
    "猎矢", "启示", "熔火", "铁誓", "青霆剑", "源石结晶", "支援晶体",
    "备料", "灼热獠牙", "爪印斫痕",
    # 团队生存状态
    "庇护",
)
...
def _extract_terms(text: str) -> list[str]:
    ...
    失衡的数值形态不计入:「N点失衡」「失衡伤害/失衡值」是失衡值机制
    (口径4,blackboard 已覆盖),非敌人身上的状态资源。"""
    ts: list[str] = []
    for m in _TERM_RE.finditer(text):
        t = m.group()
        if t == "失衡" and (text[max(0, m.start() - 1):m.start()] == "点"
                            or text[m.end():].startswith(("伤害", "值"))):
            continue
        if t not in ts:
            ts.append(t)
    return ts
...
def _post_cond_apply(text: str) -> list[str]:
    """条件句被动施加/受击:「每当有敌人被施加(冻结)」→ 被施加xx;
    捕获段以「受到」开头(「受到法术爆发伤害」)时按受击语态展示。"""
    prefix = "受到" if text.startswith("受到") else "被施加"
    return [f"{prefix}{t}" for t in _extract_terms(text)]


def _post_state(text: str) -> list[str]:
    """被字状态:「被冻结」「被(附着)源石结晶」→ 被xx。

    「施加」「强制」开头的片段(「被施加缓速」「被强制冻结」,施加动作的
    陈述或结果)交由产出表处理,此处丢弃;「被消耗的X」是定语回指,同样丢弃;
    链式片段(「冻结或被附着源石结晶」)逐词取各自的前缀。
    """
    if text.startswith(("施加", "强制", "消耗的")):
        return []
    out: list[str] = []
    rest = text
    while True:
...
def _post_demand_phrase(text: str) -> list[str]:
    ...
    之后是被施加动作(交由产出表);捕获窗跨入产出谓语(召唤/生成/施加等)
    时同样截断,后续词条归产出表。
    """
    text = re.split(r"被施加|召唤|生成|获得|返还|恢复|施加", text)[0]
    return _extract_terms(text)
...
_DEMAND_PATTERNS: list[DescPattern] = [
    ...
    # 捕获段保持贪心以维持原有 span 遮挡范围,展示由后处理在「状/的/时/后」
    # 边界截断)
...
def _attach_desc_res(skills: list[SkillRes]) -> None:
    ...
    for m in skills:
        ...
        if m.slot == "连携技":
            ...
            # 破防剔除只针对默认机制条件(进入/被施加破防,全连携技通用);
            # 保留:裸词条破防(叠层阈值)、处于破防(复合持有条件)
            m.trigger = [t for t in activation
                         if "破防" not in t or t in ("破防", "处于破防")]
        else:
            m.trigger = []
